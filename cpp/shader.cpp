...
Vec3i Shader::vertexTex(Modele *modele, int i, int n, Matrix ProjMvVp, Vec3f lumiere) {
    varying_uv.set_col(n, intToFloat(modele->getuv(i, n)));
    varying_intensity[n] = std::max(0.f, modele->normal(i, n) * lumiere);
    Vec4f gl_Vertex = embed<4>(modele->sommet(i, n));
    gl_Vertex = ProjMvVp * gl_Vertex;
    return floatToInt(proj<3>(gl_Vertex/gl_Vertex[3]));
}

bool Shader::fragmentTex(Modele *modele, Vec3f bar, TGAColor &color) {
    float intensity = varying_intensity * bar;
    Vec2f uv = varying_uv * bar;
    color = modele->couleurTexture(uv) * intensity;
    return false;
}
